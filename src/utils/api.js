...
// import {conso} from 'react-native-logger'
...
export async function get(path, params, suppressRedBox) {
  ...
  console.log('get:', path, params, full_path)
  return bodyOf(request('get', full_path, null, suppressRedBox));
}
 ...
 * @returns {Promise}  of response body
 */
export async function del(path,params,suppressRedBox) {
  let new_path = path;
  if (params){
    params = params || {}
    let params_str = _.map(params, (v,k)=>`${k}=${v}`).join('&');
    new_path = path + '?' + params_str;
  }
  return bodyOf(request('delete', new_path, null, suppressRedBox));
}
...
async function sendRequest(method, path, body) {
  ...
  try {
    ...
    const options = body
      ...
      : {method, headers};
	  console.log('api ', path, options, endpoint);
...
async function handleResponse(path, response) {
  try {
    ...
    // parse response text
	  ...
	  console.log('api response',path, resp);
	  return resp;
  } catch (e) {
    throw e;
  }
